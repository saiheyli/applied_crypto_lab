# -------- Minimal DES Implementation (64-bit block, 64-bit key) ---------
...
def permute(x, table):
    return sum(((x >> (64 - t)) & 1) << (64 - i - 1) for i, t in enumerate(table))
...
def subkeys(key):
    ...
    for s in SH:
        ...
        cd = (c << 28) | d
        keys.append(permute(cd << 8, PC2) >> 16)
    return keys
...
def main():
    print("ENTER VALUES IN HEX (without 0x)")
    pt_hex = input("Enter 64-bit plaintext  : ")
    key_hex = input("Enter 64-bit DES key    : ")

    pt = int(pt_hex, 16)
    key = int(key_hex, 16)

    cipher = des_encrypt(pt, key)
